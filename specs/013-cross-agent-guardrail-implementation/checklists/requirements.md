...
- `/speckit-plan` Phase 0 discovery (2026-07-19): this repository's own `README.md` and `.claude/settings.json` (hook paths already reference `"$HOME"/.claude/hooks/...`) revealed this repository is a distributable global configuration (`install.sh` copies `.claude/` to `~/.claude/`, applying to every project), not a self-contained project — contradicting spec 013's original project-local framing. Asked and confirmed: `AGENTS.md`, `.agents/skills/`, and the shared guardrail scripts follow the same global-installation model via an extended `install.sh` (FR-025–028, SC-011), not scoped to this repository alone. Updated Overview, User Stories 1–2, SC-004/005/007/008, Key Entities, and Assumptions accordingly. Recorded as a second `## Clarifications` entry. All checklist items re-checked against the revised spec and still pass.
