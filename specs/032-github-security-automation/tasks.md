...
- [ ] T013 [P] [US3] **Ran, result contradicts FR-008 — needs follow-up, not closing this checkbox as passing.** Pushed three scratch-branch test values: (1) AWS's own canonical placeholder key `AKIAIOSFODNN7EXAMPLE` — not blocked, and *no alert was ever created either* (no detection at all); (2) a Google API Key-shaped value (`AIza...`) — not blocked, but an async secret-scanning alert *was* created a few seconds after the push landed (confirmed via GitHub's own supported-patterns doc: Google API Key is a *detection-only* type, not enrolled in push protection, so this is expected, not a bug); (3) a fresh (non-"EXAMPLE") AWS Access Key ID/secret pair — not blocked, and **no alert was created either**, despite GitHub's docs listing AWS Access Key ID as push-protection-*and*-detection supported. (1) and (3) are the concerning cases: `secret_scanning_push_protection: enabled` did not intercept either push, and (3) didn't even generate a retroactive alert, which GitHub's own docs say it should. Root cause not yet confirmed — candidates: an allowlist for extremely common placeholder-shaped values, an org/repo-level exemption, or a real detection gap. **Follow-up needed** (out of this task's scope to resolve blind): re-test with an AWS test-mode key obtained via `gh api` `/secret-scanning` self-test tooling if one exists, or open a support inquiry, before treating FR-008 as verified for AWS-type secrets specifically. All three test branches/alerts were cleaned up (branches deleted, the Google-key alert resolved as `used in tests`).
