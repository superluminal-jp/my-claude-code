...
- **`.gitignore` lines mentioning Codex/`.codex/`/`.agents/`**: these ignore directories a developer's own tooling (OpenAI's `/import`, `migrate-to-codex`) could still create locally, independent of whether this repository documents or supports that workflow. Removing the ignore rule would not stop Codex from writing there — it would just make any such local artifact show up as untracked noise in `git status` for any developer who still runs `/import` on their own initiative. **Resolution**: keep both `.gitignore` entries (`.codex/`, `.agents/`), and reword their explanatory comment to describe the directories generically (external developer tooling this repository does not generate, deploy, or manage) without the word "Codex," and without citing a specific artifact by path (every candidate citation's own name — `specs/021-codex-official-import/`, `docs/adr/0004-*`, even the new `docs/adr/0010-*` this feature adds — contains the substring "codex," so no citation avoids reintroducing it). The literal path segment `.codex/` itself cannot avoid containing the substring "codex" — that is the external tool's own directory-naming convention, not this repository documenting or supporting it, so SC-001's repository-wide "codex" search excludes that one line on that basis while still requiring the prose comment above it to drop the word entirely.
...
- **FR-011**: No file under `specs/` outside `specs/031-remove-codex-support/` itself (this feature's own Spec Kit artifacts, which this change necessarily creates and edits) MUST be modified by this change.
...
- **SC-001**: A repository-wide case-insensitive search for "codex" — excluding `specs/`, `docs/adr/` in its entirety (every ADR body is immutable historical record; several beyond 0002/0004 — e.g. 0003, 0005, 0006 — cite Codex as part of their own historical rationale, not just the two directly superseded here), `.specify/integrations/`, `.specify/extensions/.registry`, and `.gitignore`'s literal `.codex/` directory-ignore line (not its comment; `.agents/` does not contain the substring "codex" so it was never a match) — returns zero matches.
...
- **SC-005**: `git status --porcelain specs/ ':!specs/031-remove-codex-support'` and `git status --porcelain .specify/integrations/ .specify/extensions/.registry` both return no output (no historical `specs/` directory other than this feature's own, and neither `.specify/` tree, is touched).
