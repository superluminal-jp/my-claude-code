...
6. **Given** the full remaining `tests/run-*.sh` suite set, **When** all edits above are complete, **Then** every suite exits successfully.
...
**Why this priority**: Lower priority than the mechanical removal itself (Stories 1–3), but still required before this feature is complete — the removal reverses two prior Accepted decisions (ADR-0002, ADR-0004), and this repository's own convention requires that to be recorded, not silently overwritten.

**Independent Test**: `docs/adr/` contains a new numbered ADR whose Status is Accepted and whose body names ADR-0004 (and ADR-0002, if not already superseded) as Superseded; the bodies of ADR-0002 and ADR-0004 are otherwise byte-for-byte unchanged except for their Status line.

**Acceptance Scenarios**:

1. **Given** ADR-0004 ("adopt official codex import") is currently the Accepted decision governing how this repository relates to Codex, **When** this feature is implemented, **Then** a new ADR is added with Status Accepted, and ADR-0004's Status line (only) is changed to reference it as superseding.
2. **Given** ADR-0002 ("deploy codex configuration at user scope") may or may not already be marked Superseded by ADR-0004, **When** the new ADR is written, **Then** its own Context section states ADR-0002's current status accurately rather than assuming it.
3. **Given** ADRs are immutable once Accepted, **When** ADR-0002 and ADR-0004 are touched, **Then** only their Status line changes — no other line in either file's body is edited.

### Edge Cases

- **`.gitignore` lines mentioning Codex/`.codex/`/`.agents/`**: these ignore directories a developer's own tooling (OpenAI's `/import`, `migrate-to-codex`) could still create locally, independent of whether this repository documents or supports that workflow. Removing the ignore rule would not stop Codex from writing there — it would just make any such local artifact show up as untracked noise in `git status` for any developer who still runs `/import` on their own initiative. **Resolution**: keep the `.gitignore` entries, reworded to drop the "why" prose that cites the now-deleted README section and spec, since ignoring a plausible local-tooling output directory is good hygiene independent of whether this repository documents that tooling.
...
- **FR-008**: Every remaining `tests/run-*.sh` suite MUST exit successfully after all edits are applied.
- **FR-009**: A new ADR MUST be added to `docs/adr/` with Status Accepted, documenting the decision to drop Codex CLI support entirely and naming ADR-0004 (and ADR-0002, per its actual current status) as superseded.
...
- **SC-001**: A repository-wide case-insensitive search for "codex" — excluding `specs/`, `docs/adr/0002-*`, `docs/adr/0004-*`, the new superseding ADR's required references to those two files, `.specify/integrations/`, and `.specify/extensions/.registry` — returns zero matches.
- **SC-002**: `AGENTS.md` does not exist at the repository root.
- **SC-003**: Every `tests/run-*.sh` suite exits with success.
- **SC-004**: `docs/adr/` contains exactly one new file whose Status is Accepted and whose body names the superseded ADR(s); `docs/adr/0002-*.md` and `docs/adr/0004-*.md` differ from their pre-change versions only in their Status line.
...
- **A new ADR is required**, unlike spec 029's single plugin-registration removal. This change reverses two prior Accepted architectural decisions (ADR-0002, ADR-0004) about how this repository relates to Codex — that is exactly the "architecturally significant, hard to reverse, prior decision superseded" case this repository's own ADR policy requires recording (`.claude/rules/live-documentation.md` § 0, ADR row; `adr` skill).
