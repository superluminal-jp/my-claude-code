...
- 日本語版Scrum GuideのPDFは目次分だけページ番号が英語版より後ろにずれる（sources.mdに既知）。テンプレート内の出典表記はどう扱うか？ → テンプレートはページ番号を直接埋め込まず、`[sources.md](../sources.md)` への参照と`[SG20]`タグのみを用いる。
...
- **FR-010**: 各テンプレートは `references/sources.md` への参照を含み、Scrum Guide由来の主張には `[SG20, p.X]` 形式の出典を付けること。
...
- **SC-005**: 各テンプレートの主要な記入欄に対応する `[SG20, p.X]` 形式の出典が、`sources.md` の引用ルールに沿って明示されている。
