...
- **`.claude/skills/`** — On-demand playbooks loaded by relevance: `coder`
  ...
  `scrum-master` (Scrum events, facilitation, impediments, flow metrics; team
  or solo). Spec Kit's `speckit-*`
...
| `.claude/skills/{adr,clarifier,coder,minto-builder,minto-reviewer,minto-rewriter,scrum-master}` | `~/.agents/skills/<name>` (symlink) | Native skill discovery — the same `SKILL.md`, not a copy |
...
└── .claude/                        # <-- copy this directory's contents to ~/.claude/
    ...
    │       ├── SKILL.md            #   playbook (team + solo practice)
    │       ├── references/         #   9 on-demand reference documents
