// Reminders CLI — read and write Apple Reminders through EventKit.
//
...
// Commands:
//   remind-cli lists
//   remind-cli ensure-list --name "..."
//   remind-cli list "..." [--open-only]
//   remind-cli get <identifier>
//   remind-cli create --list "..." --name "..." [--body ...] [--due ISO8601]
...
// only record of it with no recovery path the way Notes' "Recently Deleted"
// folder gives write_note.js's --delete. Deletion stays a human action in
// Reminders.app.
//
...
func authorizedStore() -> EKEventStore {
    ...
    guard granted else {
        ...
        // -- this is expected, documented behavior, not an edge case.
        fail("""
...
/// the more stable one for any reference kept beyond the current session,
/// because it survives the local `calendarItemIdentifier` changing when an
/// item moves between accounts. It is not guaranteed unique — Apple
/// documents that a recurring item can share one — so a lookup that yields
/// several is reported rather than guessed at -- this is documented, expected behavior.
...
case "update":
    ...
    // not silently create a duplicate item -- this is a documented safety requirement.
...
case "complete":
    ...
    // -- this is documented, expected framework behavior.
