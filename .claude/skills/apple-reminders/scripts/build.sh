#!/usr/bin/env bash
# Build the Reminders CLI from main.swift. Idempotent: re-running is a no-op
# unless a source file changed.
#
# Usage:
...
# source file, against a framework that ships with macOS -- the build step
# this skill accepts is a compiler call, not a dependency tree.
#
...
# Built into the skill directory and gitignored: it is a build artifact, and
# a compiled binary has no business in a diff.
BINARY="$SCRIPT_DIR/remind-cli"

if [ "$(uname -s)" != "Darwin" ]; then
  echo "build.sh: the Reminders CLI requires macOS -- EventKit is a Darwin framework." >&2
  exit 1
fi
