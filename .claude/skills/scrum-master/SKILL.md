...
Sprint Backlogやレトロ記録がClaude Codeの外——タスクアプリやノートアプリ——にあるとき、その中身を本体の会話に読み込まない。**専用のサブエージェントに委譲し、結論だけを受け取る。** リスト全件のダンプは、必要なのは結論だけなのに、以後のセッション全体のコンテキストを占有する。

本リポジトリはApple製2アプリ向けにサブエージェントを同梱している。

| サブエージェント | 担当 |
| --- | --- |
| `apple-reminders-operator` | Reminders上のSprint Backlog。項目の取得・作成・完了に加え、`item_id,started_at,completed_at`への変換と上記`flow_metrics.py`の実行まで行い、指標だけを返す |
| `apple-notes-operator` | Notes上のSprint Goal、Definition of Done、レトロ記録、障害記録の読み書き |

適用条件と境界：

- **委譲するのはデータアクセスだけで、判断は委譲しない。** Sprint Goalが作業一覧の言い換えになっていないかの検査、停滞項目の解釈、改善実験の設計、アカウンタビリティの境界——これらは本スキルの仕事として残る。サブエージェントは事実を持ってくるだけで、何も検査しない。
- **指標を受け取ったら、必ず着手記録のない項目の件数を併せて確認する。** 着手が記録されていない項目はCycle Timeの母数から落ちる。母数を伏せた中央値はSprintの中央値ではなく、透明性を損なう。
- **記録源が実際にそれらのアプリにある場合にのみ適用する。** 本セクションは本スキルのルーティングを変えない。タスク管理一般や個人の作業管理の相談が本スキルに回ってくるようになるわけではなく、[skill-routing.md](../../rules/skill-routing.md)の定義がそのまま優先する。

委譲の判断そのものの一般則は[subagent-delegation.md](../../rules/subagent-delegation.md)を参照。
...
- 外部の記録源から得た指標に、母数から落ちた項目（着手記録なし）の件数を添えているか。
- 次の検査時点と成功判定があるか。
