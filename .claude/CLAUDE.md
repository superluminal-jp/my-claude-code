...
- **Documentation Artifacts** (docstring, README, spec, OpenAPI annotation) describe current behavior; update in the **same change** as any altered public contract. Process practices — before/during/after, software-engineering and project-management disciplines — are named at `rules/live-documentation.md` § 0; its seven checks (Drift Detection, Separate-Doc-PR Detection, Auto-generation Recommendation, Proximity Enforcement, No Redundancy, Intermediate-Artifact Isolation, Granularity Layering) apply to every diff/commit/PR and every artifact created. Two of them bind hardest here. Intermediate-Artifact Isolation: final shipped code/docs must not link to Spec Kit process artifacts (`specs/NNN-*/`) — cite an ADR or plain prose instead. Granularity Layering: a fact's canonical source sits at the smallest layer where it is true (repository → subtree → directory → file → block); every artifact opens by establishing shared ground and states its answer before the supporting detail; and an artifact may rely on terms introduced at a higher layer, never on terms defined only below it.
