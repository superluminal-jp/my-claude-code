#!/usr/bin/env bash
# Shared destructive-command guardrail (Q6). Tool-agnostic: reads {"command": "..."}
# on stdin, writes {"decision": "allow"|"deny"|"ask", "reason": "..."} on stdout.
# Exit 0 regardless of decision; non-zero only on a real script error (caller
# must treat that the same as "deny", per contracts/guardrail-script-io.md).
#
# Extracted from .claude/hooks/pre-bash.sh — logic and messages kept verbatim
# so both the refactored Claude Code hook and the Codex CLI adapter produce
# identical outcomes to what pre-bash.sh already produced (FR-009).
...
if echo "$COMMAND" | grep -qE 'git push.*(\s-f(\s|$)|--force)'; then
  emit "deny" "Force push is blocked by policy. Use a normal push or get explicit user approval."
  exit 0
fi

# Block hard reset
if echo "$COMMAND" | grep -qE 'git reset.*--hard'; then
  emit "deny" "git reset --hard is blocked by policy. Confirm with user before discarding changes."
  exit 0
fi

# Block git clean -f (force delete untracked files)
if echo "$COMMAND" | grep -qE 'git clean.*-[a-zA-Z]*f'; then
  emit "deny" "git clean -f is blocked by policy. Confirm with user before deleting untracked files."
  exit 0
fi
...
if echo "$COMMAND" | grep -qE '(^|;|&&|\|\||[|(&])\s*rm\s+-[a-zA-Z]*r[a-zA-Z]*f|(^|;|&&|\|\||[|(&])\s*rm\s+-[a-zA-Z]*f[a-zA-Z]*r'; then
  ...
  if echo "$COMMAND" | grep -qE '(^|;|&&|\|\||[|(&])\s*rm\s+(-[a-zA-Z]+\s+)*(\/|~\/?|\.\/?|\/\*)(\s|$)' ||
    ...
    emit "deny" "rm -rf targeting root, home, or current directory is permanently blocked by policy."
    exit 0
  fi
  # All other rm -rf: route through user confirmation
  emit "ask" "rm -rf requires explicit user approval before deleting directories"
  exit 0
fi
...
if echo "$COMMAND" | grep -qE '>\s*/dev/sd[a-z]' ||
  ...
  emit "deny" "System-destructive command blocked by policy."
  exit 0
fi

# Block curl | bash / wget | bash patterns
if echo "$COMMAND" | grep -qE '(curl|wget).*\|.*(bash|sh|zsh)'; then
  emit "deny" "Piping remote scripts to shell is blocked by policy."
  exit 0
fi
...
if echo "$COMMAND" | grep -qE '(curl|wget)'; then
  while IFS= read -r url; do
    ...
    emit "deny" "Non-HTTPS HTTP requests are blocked by policy (except localhost)."
    exit 0
...
if echo "$COMMAND" | grep -qE "${READ_CMD_RE}\s+" &&
  ...
  if echo "$COMMAND" | grep -qE '(/\.ssh/|/\.aws/|~/\.ssh|~/\.aws|\.env(\s|$|\.)|secrets/|credentials/|\.pem(\s|$)|\.p12(\s|$)|\.pfx(\s|$))'; then
    emit "deny" "Reading credential paths or key material via shell is blocked by policy."
    exit 0
  fi
...
if echo "$COMMAND" | grep -qE '(>|>>|\|\s*tee(\s|$))'; then
  if echo "$COMMAND" | grep -qE '(/\.ssh/|/\.aws/|~/\.ssh|~/\.aws|\.env(\s|$|\.)|\.pem(\s|$)|\.p12(\s|$)|\.pfx(\s|$))'; then
    emit "deny" "Writing to credential paths or key material is blocked by policy."
    exit 0
  fi
fi
...
if echo "$COMMAND" | grep -qE '(^|;|&&|\|\||[|(&])\s*sudo(\s+-\S+)*\s+pip3?\s+install\b' ||
  ...
  emit "deny" "$GLOBAL_INSTALL_MSG (pip: use a virtual environment instead of --user/sudo)"
  exit 0
fi

# uv pip install --system
if echo "$COMMAND" | grep -qE '(^|;|&&|\|\||[|(&])\s*uv\s+pip\s+install\b' && echo "$COMMAND" | grep -qE '(^|\s)--system(\s|$)'; then
  emit "deny" "$GLOBAL_INSTALL_MSG (uv: omit --system, install into the project venv)"
  exit 0
fi

# npm/pnpm: -g / --global
if echo "$COMMAND" | grep -qE '(^|;|&&|\|\||[|(&])\s*(npm|pnpm)\s+(install|i|add)\b' && echo "$COMMAND" | grep -qE '\s(-g|--global)(\s|$)'; then
  emit "deny" "$GLOBAL_INSTALL_MSG (npm/pnpm: drop -g/--global, add as a project dependency)"
  exit 0
fi

# yarn global add
if echo "$COMMAND" | grep -qE '(^|;|&&|\|\||[|(&])\s*yarn\s+global\s+add\b'; then
  emit "deny" "$GLOBAL_INSTALL_MSG (yarn: use 'yarn add' inside the project instead of 'yarn global add')"
  exit 0
fi
...
if echo "$COMMAND" | grep -qE '(^|;|&&|\|\||[|(&])\s*gem\s+install\b' && ! echo "$COMMAND" | grep -qE '(^|\s)--user-install(\s|$)'; then
  emit "deny" "$GLOBAL_INSTALL_MSG (gem: add --user-install, or add the gem to the project's Gemfile via bundler)"
  exit 0
fi
...
if echo "$COMMAND" | grep -qE '(^|;|&&|\|\||[|(&])\s*cargo\s+install\b' && ! echo "$COMMAND" | grep -qE '(^|\s)--path(\s|$)'; then
  emit "deny" "$GLOBAL_INSTALL_MSG (cargo: use --path . for project-local builds, or add the crate to Cargo.toml as a dependency)"
  exit 0
fi

# Route sudo through user confirmation
if echo "$COMMAND" | grep -qE '(^|[;&|[:space:]])sudo(\s|$)'; then
  emit "ask" "sudo requires explicit user approval"
  exit 0
fi
