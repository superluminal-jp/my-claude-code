...
| `.claude/skills/*`(手書き 11 件 + `speckit-*` 15 件、プロジェクトスコープ。`digital-agency-frontend`・`scrum-master`・`apple-notes`・`apple-reminders` を含む) | `.agents/skills/*`(リンクは 10 件) | 移植済み(013)、拡張(015/016/019) | 単一ソース | 相対シンボリックリンク。`verify-config`(019) は Codex 側の実体が `.codex/prompts/verify-config.md` のためリンクを張らない。各スキルの `references/`・`agents/`・`scripts/` も同じリンク先から参照。シンボリックリンク非対応環境は対象外 | research R2; 015 research R1/R7; 016 research R4; SYNC-01 |
| `.claude/skills/*`(ユーザースコープ展開、手書き 11 件。`digital-agency-frontend`・`scrum-master`・`apple-notes`・`apple-reminders` を含む) | `~/.agents/skills/*` → `~/.claude/skills/*` | 本機能で移植(014)、拡張(015/016/019/ADR 0004) | 単一ソース | `install.sh` が展開先を指すリンクを再生成。`~/.claude/skills/` は 11 件、`~/.agents/skills/` のリンクは `CUSTOM_SKILLS` の 10 件（`verify-config` を除く）。削除済み 3 スキルは 33c82eb に追従して除外 | research R0/R2; 015 FR-016; 016 FR-011; SYNC-02 |
| `.claude/skills/verify-config/SKILL.md` | `.codex/prompts/verify-config.md` → `~/.codex/prompts/verify-config.md` | 本機能で移植(014)、移設(019) | 検証付き二重管理 | `/prompts:verify-config` として明示起動。Claude 側は 019 で `.claude/commands/` から skill へ移設し `context: fork` を付与。Codex に forked context 相当はないため、prompt は同じ検査手順をインラインで実行する | research R7; FR-008; SYNC-09; 019 research R2 |
| `.claude/agents/verification-runner.md` | — | 対象外 | — | Claude Code の subagent 定義。Codex に独立コンテキストの subagent 相当がなく、`/prompts:verify-config` が同じ手順を本体セッションで実行するため移植しない。読取専用の tool allowlist も Codex 側は Rules で代替 | 019 plan D1/D2/D4 |
| `.claude/agents/apple-notes-operator.md`・`.claude/agents/apple-reminders-operator.md` | — | 対象外(定義)／移植済み(スキル本体) | — | Claude Code の subagent 定義。Codex に独立コンテキストの subagent 相当がないため定義は移植しない。ただし各エージェントが `skills:` で読み込む `apple-notes`／`apple-reminders` スキルは自己完結しており、`~/.agents/skills/` 経由で Codex からも使える。既知差分は Codex 側で出力隔離と tool allowlist（Edit/Write 不可）が得られないこと | ADR 0004 |
