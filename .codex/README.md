...
| `.claude/skills/*`(手書き 9 件 + `speckit-*` 15 件、プロジェクトスコープ。`digital-agency-frontend`・`scrum-master` を含む) | `.agents/skills/*`(リンクは 8 件) | 移植済み(013)、拡張(015/016/019) | 単一ソース | 相対シンボリックリンク。`verify-config`(019) は Codex 側の実体が `.codex/prompts/verify-config.md` のためリンクを張らない。各スキルの `references/`・`agents/`・`scripts/` も同じリンク先から参照。シンボリックリンク非対応環境は対象外 | research R2; 015 research R1/R7; 016 research R4; SYNC-01 |
| `.claude/skills/*`(ユーザースコープ展開、手書き 9 件。`digital-agency-frontend`・`scrum-master` を含む) | `~/.agents/skills/*` → `~/.claude/skills/*` | 本機能で移植(014)、拡張(015/016/019) | 単一ソース | `install.sh` が展開先を指すリンクを再生成。`~/.claude/skills/` は 9 件、`~/.agents/skills/` のリンクは `CUSTOM_SKILLS` の 8 件（`verify-config` を除く）。削除済み 3 スキルは 33c82eb に追従して除外 | research R0/R2; 015 FR-016; 016 FR-011; SYNC-02 |
