...
| `.claude/skills/*`(手書き 7 件 + `speckit-*` 15 件、プロジェクトスコープ) | `.agents/skills/*` | 移植済み(013) | 単一ソース | 相対シンボリックリンク。シンボリックリンク非対応環境は対象外 | research R2; FR-007; SYNC-01 |
| `.claude/skills/*`(ユーザースコープ展開、手書き 7 件) | `~/.agents/skills/*` → `~/.claude/skills/*` | 本機能で移植(014) | 単一ソース | `install.sh` が展開先を指すリンクを再生成。削除済み 3 スキルは 33c82eb に追従して除外 | research R0/R2; FR-010; SYNC-02 |
