...
| `.claude/skills/*`(手書き 7 件 + `speckit-*` 15 件、プロジェクトスコープ。`digital-agency-frontend` を含む) | `.agents/skills/*` | 移植済み(013)、拡張(015) | 単一ソース | 相対シンボリックリンク。各スキルの `references/` と `agents/` も同じリンク先から参照。シンボリックリンク非対応環境は対象外 | research R2; 015 research R1/R7; SYNC-01 |
| `.claude/skills/*`(ユーザースコープ展開、手書き 7 件。`digital-agency-frontend` を含む) | `~/.agents/skills/*` → `~/.claude/skills/*` | 本機能で移植(014)、拡張(015) | 単一ソース | `install.sh` が展開先を指すリンクを再生成。削除済み 3 スキルは 33c82eb に追従して除外 | research R0/R2; 015 FR-016; SYNC-02 |
