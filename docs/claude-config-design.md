...
構成は `CLAUDE.md` を唯一の頂点とする 3 層のピラミッドとして保つ。第 2 層は 8 個のルール、第 3 層はそのルールに対応する 8 個の自作スキルである。各ファイルは上位層だけに依存し、同一階層のファイルを参照しない。

| 層 | 依存できる上位層 | 許可する参照 | 禁止する参照 |
|---|---|---|---|
| `CLAUDE.md` | なし | 子であるルールへの下向きルーティング | なし |
| `.claude/rules/*.md` | `CLAUDE.md` | 対応する子スキルへの下向きルーティング | 別のルールのファイル名・パス・それと分かる言及 |
| ルールに対応する 8 個の自作 `SKILL.md` | `CLAUDE.md` と担当ルール | 自身の手順に必要な外部資料 | 対象 8 スキル間の名前付き言及 |

対象スキルは `clarifier`、`coder`、`adr`、`minto-builder`、`minto-reviewer`、`minto-rewriter`、`scrum-master`、`digital-agency-frontend` に限定する。ルール名とスキル名が同じ場合でも、子スキルを指す下向き参照は許可する。禁止するのは、同じ層にある別ファイルを指す横向き参照である。
