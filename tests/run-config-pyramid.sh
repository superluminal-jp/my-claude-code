...
run_routing_fixtures() {
  ...
  check_contains "ROUTE-11: decomposition owns slicing only, not the sliced specification" "$SKILL_DIR/meta-spec/SKILL.md" '^description:.*(decompos|slice).*(out of scope|not for|対象外|使わない)'
}
